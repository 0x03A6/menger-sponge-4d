...
struct PathRecorder {
    ...
    void correctCamInfo(CamInfo &info) {
        ...
        info.head -= info.front * dot(info.head, info.front);
        info.head.normalize();
        info.somewhere = cross(info.front, info.right, info.head);
    }
    ...
    void calcPath() {
        ...
        for (double i = original_path.begin()->t; i < t_total; i += 1.0 / 25.0) {
            ...
            path.push_back({
                ...
                it->cam.head * v1 + (it + 1)->cam.head * v2,
                it->cam.somewhere * v1 + (it + 1)->cam.somewhere * v2   // 这个参数疑似啥值都可以
            });
    ...
    void smoothenPath() {
        for (int i = 0; i < 20; i++) {
            ...
            for (int j = 1; j < path.size() - 1; j++) {
                ...
                temp_path[j].head = path[j].head * 1 + path[j - 1].head * 1 + path[j + 1].head * 1;
                temp_path[j].somewhere = path[j].somewhere * 1 + path[j - 1].somewhere * 1 + path[j + 1].somewhere * 1; // 疑似可删
    ...
    void generateImage(Renderer &renderer) {
        for (int i = 0; i < path.size(); i++) {
            ...
            renderer.camera.somewhere = path[i].somewhere;
    ...
    void showPath(Renderer &renderer) {
        for (int i = 0; i < path.size(); i++) {
            ...
            renderer.camera.right = path[i].right;
            renderer.camera.somewhere = path[i].somewhere;
            // renderer.camera.head.print();   // DEBUG
            // renderer.camera.front.print();  // DEBUG
            // renderer.camera.right.print();  // DEBUG
            // renderer.camera.somewhere.print();  // DEBUG
            // putchar('\n');  // DEBUG
            calcInitBlock(renderer.camera.pos);
            renderer.camera.relative_pos = enterInitBlock(renderer.camera.pos);
            renderer.render();
