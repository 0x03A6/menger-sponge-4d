...
inline void mainLoop(Renderer &renderer) {
    ...
    while (true) {
        ...
        if (GetAsyncKeyState('E') & 0x8000) renderer.camera.rotateWPositive(move_delta), moved = true;
        if (GetAsyncKeyState('Q') & 0x8000) renderer.camera.rotateWNegative(move_delta), moved = true;
